...
function getOnesignalId() {			  
  OneSignal.push(function() {
    ...
    OneSignal.isPushNotificationsEnabled().then(function(isPushEnabled) {
      OneSignal.getUserId(async function(onesignal_id) {
        console.log(onesignal_id);
    
        try {
          let data = new FormData()
          data.append('user_id', app.current_user.type_id)
          data.append('onesignal_id', onesignal_id)
          data.append( 'user_type', app.current_user.type) 
          let req = await fetch('../api/save_onesignalId.php',{
            method:'POST',
            body:data
          })
...
function login_app() {
  ...
  $('#btn_login').on('click',async function() {
    ...
    try {
      ...
      data.append('user_password', $('input[name=user_password]').val())
      data.append( 'submit', 'submit') 
      let req = await fetch('server/http_request/login.php',{
        method:'POST',
        body:data
      })
      let res = await req.json()
      ...
      } else if (res.success == false && res.error == 'C1') {
        alert('Contraseña incorrecta');
      } else if (res.success == false && res.error == 'C2') {
        alert('Ingrese una contraseña');
      } else if (res.success == false && res.error == 'U1') {
        alert('Email incorrecto');
      } else if (res.success == false && res.error == 'U2') {
...
async function info_user() {
  ...
  try {
    let req = await fetch('server/http_request/info_user.php',{
      method:'POST'
    })
    let res = await req.json()
    if (res) {
      ...
      console.log(res);
      getSumary(res.type_id)
      getSumary2(res.type_id)
      getOTS(res.type_id)
      loadInfoServicesPanel2(res.type_id)
      get_expontaneos(res.type_id)
      get_order_per_type(res.type_id,"correctivo")
      get_order_per_type(res.type_id,"urgente")
      get_order_per_type(res.type_id,"preventivo")
      $("#info_user_log").text(JSON.stringify(res))
      get_locationClient();
    } else {
...
async function units() {
  ...
  try {
    let req = await fetch('../api/get_units.php',{
      method:'POST'
    })
    let res = await req.json()
    if (res.success) {
...
function testCoordenadas() {
  $("#test").on("click", async () => {
    ...
    try {
      ...
      data.append('id',id)
      let req = await fetch('server/http_request/coordinates.php',{
        method:'POST',
        body:data
      })
      let res = await req.json()
...
function fullCalendar(){
  ...
  $("#btn-panel-agenda, #agenda_intro, #btn-agenda").on("click", function() {
    ...
    if (doConnection() ) {
      ...
      fetch("../api/get_orders.php?empleado_id="+ empleado_id)
        .then( data => data.json())
        .then(function(response) {
          if (response.success == true) {
            ...
            calendar.funcPer = function(ev){
              ...
              fetch(`../api/get_orders.php?empleado_id=${empleado_id}&fecha=${ev.currentSelected}`)
              .then(data=> data.json())
              .then(function(response) {
                  if (response.success) {
                    $('#loader').css('display', 'none');
...
function into_agenda() {
  ...
  fetch(`../api/check_employee_hour.php?id_empleado=${id_empleado}`)
    .then(data => data.json())
    .then(function(response) {
      if (response.success) {
        $('#loader').css('display', 'none');
...
function goIntro(){
  ...
  $('.btn-listPedidos, .btn-intro-espont').off('click').on('click', function() {
    ...
    $('#loader').css('display', 'flex');
    fetch(`../api/get_client_orders.php?id_cliente=${id_cliente}&tipo=${type_otc}&locations=${app.current_user.locations}`)
      .then(data => data.json())
      .then(function(response) {
        if (response.success == true) {
          $('#loader').css('display', 'none');
  ...
  $('.btn-intro-order').on('click', ()=>{
    ...
    var empleado_id = app.current_user.type_id;
    if (doConnection()) {
      fetch("../api/get_ots_summary.php?empleado_id="+ empleado_id)
      .then(res => res.json())
      .then(function(response) {
        if (response.success) {
          $('#loader').css('display', 'none');
      ...
      seePanel_ots();
      var value_parse1 = $("#info_sumary_log").text();
      if(value_parse1 != "") var info = JSON.parse(value_parse1);
      $('#loader').css('display', 'none');
          
      document.getElementById('cant_agenda').innerHTML = info.agenda;
      document.getElementById('cant_servicios').innerHTML = info.servicios;
      document.getElementById('cant_preventivo').innerHTML = info.OCs;
      document.getElementById('cant_obras').innerHTML = info.obras;  
    }
  ...
  $('#btn-panel-materiales').off('click').on('click', ()=>{
    $('#loader').css('display', 'flex');
    fetch("../api/get_purchaseOrders.php?id_empleado="+app.current_user.type_id)
    .then(res => res.json())
    .then(function(response) {
      $('#loader').css('display', 'none');
      var data_purchase = '';
  ...
  $('.btn_panel_correctivo, .btn_panel_urgente, .btn_panel_preventivo, #link_hoja , .btn_panel_obra').on('click', function(){
    ...
    if (doConnection()) {
      ...
      fetch(`../api/get_orders.php?empleado_id=${empleado_id}&fecha=${fecha}&clasificacion=${typeOt[2]}`)
      .then(res => res.json())
      .then(function(response) {
        if (response.success == true) {
          $('#loader').css('display', 'none');
      ...
      let name = `info_${typeOt[2]}_log`
      let info = JSON.parse($("#"+name).text())
      $('#loader').css('display', 'none');
      console.log(info);
      var data_order = '';
      var importancia = '';
      ...
      for (var i = 0; i < info.orders.length; i++) {
        ...
        if (info.orders[i].order_item_pendings == true) {
          pendiente = '<div class="item_pendiente"></div>';
        } else {
          pendiente = '';
        }

        if (info.orders[i].locacion_nombre == null) {
          info.orders[i].locacion_nombre = '';
        }

        if (info.orders[i].locacion_direccion == null) {
          info.orders[i].locacion_direccion = info.orders[i].direccion;
        }

        if (info.orders[i].order_items > 0 && info.orders[i].order_items == info.orders[i].order_items_finished) {
          back_ot = '#e7e7e7';
          txt_importancia = 'Ejecutada';
        } else {
          back_ot = '#fff';
        ...
        var start = info.orders[i].fecha+' '+info.orders[i].hora_desde;
        var end = info.orders[i].fecha+' '+info.orders[i].hora_hasta;
        Ttime_Ot =  moment.utc(moment(end,"YYYY-MM-DD HH:mm").diff(moment(start,"YYYY-MM-DD HH:mm"))).format("HH:mm");

        var horaDesde = info.orders[i].hora_desde.slice(0, -3);
        var horaHasta = info.orders[i].hora_hasta.slice(0, -3);

        turno_tipo = info.orders[i].turno_tipo;
        change_turnoTipo(turno_tipo);
        ...
        data_order +=  `<div style="background: ${back_ot}" class="${class_ot}" id="`+ info.orders[i].id_orden +`">
                          <p id="empresa_ot">`+ info.orders[i].razon_social +`</p>
                          <p>`+ info.orders[i].descripcion +`</p>
                          <p id="lugar_ot">`+ info.orders[i].locacion_nombre +`</p>
                          <p class="hidden_input"  id="lugar2_ot">`+ info.orders[i].locacion_direccion +`</p>
                          <p class="hidden_input" id="loc_ot">`+ info.orders[i].locacion_localidad +`</p>
                          <p class="hidden_input" id="prov_ot">`+ info.orders[i].locacion_provincia +`</p>
                          <p id="cod_ot">OT `+ info.orders[i].id_ot +`</p>
                          <p id="time_ot">`+horaDesde+`hs. a `+horaHasta+`hs</p>
                          <p class="type_duration"><span id="duration_job">`+Ttime_Ot+`hs.</span><br><span id="type_job">${turno_tipo}</span></p>
                          <div class="caja_status">
                              <span style='margin-right: 5px; font-weight: 700;'>${txt_importancia}</span>
  ...
  $('#btn-panel-agenda, #agenda_intro, #btn-agenda').on('click', ()=>{
    ...
    if ( doConnection() ) {
      ...
      fetch(`../api/get_orders.php?empleado_id=${empleado_id}&fecha=${fecha}`)
      .then(res => res.json)
      .then( function(response) {
        if (response.success == true) {
          $('#loader').css('display', 'none');
      ...
      let info = JSON.parse($("#info_sumary2_log").text())
      if (info.success == true) {
        ...
        for (var i = 0; i < info.orders.length; i++) {
          ...
          data_order +=  `<div style="background: ${back_ot}" class="div_ot_agendaI" id="`+ info.orders[i].id_orden +`">
                            <p id="empresa_ot">`+ info.orders[i].razon_social +`</p>
                            <p id="lugar_ot">`+ info.orders[i].locacion_nombre +`</p>
                            <p id="lugar2_ot">`+ info.orders[i].locacion_direccion +`</p>
                            <p id="cod_ot">OT `+ info.orders[i].id_ot +`</p>
                            <p id="time_ot">`+horaDesde+`hs. a `+horaHasta+`hs</p>
                            <p class="type_duration"><span id="duration_job">`+Ttime_Ot+`hs.</span><br><span id="type_job">${turno_tipo}</span></p>
                            <div class="caja_status">
        ...
        showCalendar();
        $('#loader').css('display', 'none');
        setInterval(() => $('#loader').css('display', 'none'), 3000);
    } else {
      $('.main-agenda').html('<p class="message_error">No hay Orenes de Trabajo para hoy.</p>');
      $('#loader').css('display', 'none');
      seePanel_agenda();
...
function loadInfoServicesPanel() {
  ...
  if ( doConnection() ) {
    ...
    var empleado_id = app.current_user.type_id;
    var fecha = moment().format('YYYY-MM-DD');
    fetch(`../api/get_orders_summary.php?empleado_id=${empleado_id}&fecha${fecha}`)
    .then(res => res.json())
    .then(function(response) {
      if (response.success == true) {
        $('#loader').css('display', 'none');
    ...
    let info = JSON.parse($("#info_services_panel").text())
    $('#loader').css('display', 'none');
    console.log(info);
    document.getElementById('cant_urgentes').innerHTML = info.urgente;
    document.getElementById('cant_preventivos').innerHTML = info.preventivo;
    document.getElementById('cant_correctivos').innerHTML = info.correctivo;
    document.getElementById('cant_espontaneos').innerHTML = info.espontaneo;
    seePanel_servicios();
  }
...
function iniciar_jornada() {
  ...
  if (jornada == 'open') {
    ...
    $(".btn_ingresar_clave").click(function(){
      var id_empleado = app.current_user.type_id;
      $('#loader').css('display', 'flex');
      fetch(`../api/check_employee_hour.php?id_empleado=${id_empleado}&check=true`)
        .then(res => res.json())
        .then(function(response) {
          $('#loader').css('display', 'none');
          if (response.success) {
        ...
        $("#btn_start").click(function() {
          if ($('#claveG_inicio').val() == app.current_user.clave) {
            ...
            .then(function(position) {
              ...
              localStorage.setItem('coodenadas',coordenadas)
              $('#loader').css('display', 'flex');

              let data = new FormData()
              data.append('id_empleado',id_empleado)
              data.append('tipo',tipo)
              data.append('coordenadas',coordenadas)
              fetch(`../api/save_employee_hour.php`,{
                method:'POST',
                body:data
              })              
              .then(res => res.json())
...
function replanteos_pedidos() {
  ...
  fetch("../api/get_clients.php?id_empleado="+app.current_user.type_id)
  .then(res => res.json())
   .then(function(response) {
      if (response) {
  ...
  $('#cliente_repl').off('change').on('change', function() {
    ...
    fetch("../api/get_contacts.php?id_cliente="+id_client)
    .then(res => res.json())
    .then(function(response) {
      if (response) {
  ...
  function changeInputLocation() {
    ...
    fetch(`../api/get_locations.php?id_contacto=${id_contact}`)
    .then(res => res.json())
    .then(function(response) {
      $('#loader').css('display', 'none');
      if (response) {
...
function seePanel_listEspontaneos() {
  ...
  $('#new_espontaneo').off('click').on('click', function() {
    ...
    $('#cliente_espon').on('change', function() {
      var id_client = $(this).val();
      fetch(`../api/get_locations.php?id_cliente=${id_client}`)
      .then(res => res.json())
        .then(function(response) {
          $('#loader').css('display', 'none');
          if (response) {
...
function seePanel_listPedidos() {
  ...
  $('.bt_doFilter button').off('click').on('click', function() {
    ...
    var id_cliente = app.current_user.type_id;
    $('#loader').css('display', 'flex');
    fetch(`../api/get_client_orders.php?id_cliente=${id_cliente}&exec=${fStatusOt}&id_locacion=${fLocationOt}&locations=${app.current_user.locations}`)
    .then(res => res.json())
    .then(function(response) {
        $('#loader').css('display', 'none');
        if (response) {
...
function seePanel_vistaOt() {
  ...
  $('.div_ot_agenda').click(function() {
    ...
    if ( doConnection() ) 
      ...
      //toast: Trabajando sin conexión, cuidado!
...
function seePanel_replanteo_sinOt() {
  $('#loader').css('display', 'flex');
  fetch("../api/get_replanteos.php?id_empleado="+app.current_user.type_id)
  .then(res => res.json())
  .then(function(response) {
    var data_order = '';
    if (response.success == true) {
...
function seePanel_espontaneos() {
  ...
  if(doConnection()){
    ...
    let info = JSON.parse($("#info_espontaneos_log").text())
    console.log(info);
    var data_order = ''
    var importancia = '';
    var txt_importancia = '';
    ...
    if (info.orders[i].locacion_nombre == null) {
        info.orders[i].locacion_nombre = info.orders[i].direccion;
    }
    ...
    if (type_otc == 'ot-espontaneo' && info.orders[i].apro_cliente == 'f') {
      status_esp = '<i style="color: #dc0000;" class="fas fa-check-circle esp_status"></i>';
    } else if (type_otc == 'ot-espontaneo' && info.orders[i].apro_cliente == 't'){
      status_esp = '<i style="color: green;" class="fas fa-check-circle esp_status"></i>';
    } else {
      status_esp = '';
    }

    data_order +=  `<div style="background: ${back_ot}" class="single_espon" id="`+ info.orders[i].id_orden +`">
                        <p id="empresa_ot">`+ info.orders[i].razon_social +`</p>
                        <p id="lugar_ot">`+ info.orders[i].locacion_nombre +`</p>
                        
                        <p class="text_resaltado">`+ info.orders[i].descripcion +`</p>
                        <p id="cod_ot">OT `+ info.orders[i].id_ot +`</p>
                        <div class="caja_status">
                            <span style='margin-right: 5px; font-weight: 700;'>${txt_importancia}</span>
                            <div style="background: #${importancia};" class="iportancia_ot"></div>
                        ...
                        <span id="date_otClient">${info.orders[i].fecha}</span>
                        ${status_esp}
...
function get_items_a(response) {
  console.log('get_items_a',response)
  id_clientSign = response.id_cliente;
  var importancia = '';
  var pendiente = '';
  var txt_importancia = '';
  ...
  if (response.order_item_pendings == true) {
    pendiente = '<div class="item_pendiente"></div>';
  } else {
    pendiente = '';
  }
  if (response.locacion_nombre == null) {
    response.locacion_nombre = '';
  }
  if (response.locacion_direccion == null) {
    response.locacion_direccion = response.direccion;
  }

  var data_order = `<img src="assets/images/icon/operarios.png" alt="">
                    <p id="empresa_ot">`+ response.razon_social +`</p>
                    <p class="cod-ots">OT `+ response.id_ot +`</p>
                    <p>`+ response.locacion_nombre +`</p>
                    <p class="cod-ots">`+ response.descripcion +`</p>
                    <p class="type_duration"><span id="duration_job">`+Ttime_Ot+`hs.</span><br><span id="type_job">${turno_tipo}</span></p>
  ...
  var data_orderRes =  `<img src="assets/images/icon/operarios.png" alt="">
                        ...
                        <p class="cod-ots">OT `+ response.id_ot +`</p>
                        <p>`+ response.locacion_nombre +`</p>`;
                        // <p class="text_resaltado" id="desc_ot">`+ response.locacion_direccion +`</p>`;

  var data_orderRep =  `<img src="assets/images/icon/operarios.png" alt="">
                        <h5>`+ response.razon_social +`</h5>
                        <p>`+ response.locacion_nombre +`</p>`;

  $('.ot_code').html('OT '+ response.id_ot);
  $('.main_info_ots').html(data_order);
  $('.main_info_ots_resumen').html(data_orderRes);
  $('.info_ots').html(data_orderRep);
  modal_infoPedido(response.descripcion, response.detalle);
  ...
  console.log(12312)
  if ((response.order_items.length > 0 && response.order_items.length == response.order_items_finished && estado_ot == 'closed') || action_ot == 'watch') {
    $('.fix_button').css('display', 'none');
    console.log(1)
  } else if (response.order_items.length > 0 && response.order_items.length != response.order_items_finished && estado_ot == 'open') {
...
function get_items() {
  ...
  if (doConnection()) { 
    ...
    var a = JSON.parse($("#info_ot_log").text())

    a.order.forEach(or => {
      console.log('or',or)
      if (orden_id == or.id_orden)  {
        if ( or.order_items.length>0 ) {
          //fx 2
          var arreglo_items = {};
          arreglo_items['success'] = "true";
          arreglo_items['items'] = or.order_items;

          get_items_b(arreglo_items);
        } else {
          var data_item =  `<p class="message_error">No hay items en ésta Orden de trabajo.</p>`;
          $('.all_items').html(data_item);
          $('#loader').css('display', 'none');
...
function process_startOt() {
  ...
  $('.bt_inicia_ot').click(function() {
    if(doConnection()){
      ...
      let info = JSON.parse($("#info_user_log").text())
      if ($('#clave_openItem').val() == info.key){
...
function open_modals() {
  ...
  $('.realiza_ots').on('click', function() {
    ...
    if( doConnection() ) $.ajax({
      ...
      //not finished, need to do all the corrections.
      process_startOt();
    }//else no connection
...
function slider_correctivos() {
  ...
  $("#btn_fin_item").click(function() {
    ...
    var tipo = 'it.1';
    var id_empleado = app.current_user.type_id;
    fetch(`../api/get_order.php?orden_id=${orden_id}`)
     .then(res => res.json())
      .then(function(response) {
        if (response.success == true) {
          getStatusItem = response.order.order_items.find(item => item.id_orden_item == id_item);
  ...
  $(".leave_item").off('click').on('click', function() {
    ...
    $('#bt_leave_ok_step').off('click').on('click', function() {
      ...
      let data = new FormData()
      data.append('id_gente', id_leaveGente)
      data.append('id_empleado', app.current_user.type_id )

      fetch("../api/leave_item_gente.php",{
        method: "POST",
        body:data
      })
       .then( res => res.json())
...
function get_locationClient() {
  ...
  fetch("../api/get_locations.php?id_contacto="+id_cliente)
  .then(res => res.json())
  .then(function(response) {
    $('#loader').css('display', 'none');
    if (response.success == true) {
...
function getSumary(type_id){

  fetch("../api/get_ots_summary.php?empleado_id="+type_id)
  .then(res => res.json())
  .then( (response) =>{
    console.log("getsummary: "+response);
    $("#info_sumary_log").text(JSON.stringify(response))
  })
  .catch(err => err)  
}

function getSumary2(type_id){
  let fecha = new Date().toJSON().slice(0,10)
fetch(`../api/get_orders.php?empleado_id=${type_id}&fecha=${fecha}`)
.then(res => res.json())
.then( function(response) {
    console.log("getsummary2: "+response);
    $("#info_sumary2_log").text(JSON.stringify(response))
  })
  .catch(err => err)  
}

function getOTS(type_id){
  var array_response = {};
  fetch("../api/get_orders.php?empleado_id="+ type_id)
  .then( data => data.json())
  .then(function(response) {
    console.log('orders',response)

    array_response["success"] = true;
    array_response["order"] = [];

    var array_n = 0;

    response.orders.forEach(res1 => {
      fetch("../api/get_order.php?orden_id="+res1.id_orden)
      .then( data => data.json())
      .then(function(response2) {
        // console.log('order',response2)
        array_response["order"][array_n] =  response2.order;
        array_n++;
      })
      .then( () => {
        console.log("array response ots",array_response);    
        $("#info_ot_log").text(JSON.stringify(array_response));
      })
      .catch(err=>err )
    });
...
const checkConn = () => {
  ...
  setInterval( () =>{
    if (doConnection()) {
      $('#btn_sinc').css('display',"none")
      console.log('+++++++')   
    }else{
      $('#btn_sinc').css('display',"block")  
      console.log('*******')   
    }
  }, 1000);
...
function loadInfoServicesPanel2(type_id) {
  var fecha = moment().format('YYYY-MM-DD');
  fetch(`../api/get_orders_summary.php?empleado_id=${type_id}&fecha${fecha}`)
  .then(res => res.json())
  .then(function(response) {
    $("#info_services_panel").text(JSON.stringify(response))
  })    
  .catch(function() {
    console.log('error ajax -> get_orders_summary.php');
    $('#loader').css('display', 'none');
...
function get_order_per_type(type_id,tipo) {
  var fecha = moment().format('YYYY-MM-DD');
  fetch(`../api/get_orders.php?empleado_id=${type_id}&fecha=${fecha}&clasificacion=${tipo}`)
  .then(res => res.json())
  .then(function(response) {
    $("#info_"+tipo+"_log").text(JSON.stringify(response))
  })
}

function get_expontaneos(type_id){
  fetch(`../api/get_espontaneos.php?id_empleado=`+type_id)
  .then(res => res.text())
  .then(function(response) {
    $("#info_espontaneos_log").text(response)
  })
}