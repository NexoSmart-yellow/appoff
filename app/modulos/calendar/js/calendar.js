...
class CalendarYvv{
	constructor(etiqueta="", diaSeleccionado="", primerDia="Lunes") {
		...
		for (var i = 0; i < this.diasResal.length; i++) {
		...
		this.textResalt = "#212529"; // color de texto de dias importantes
	...
	startElements(){
		...
		this.diasCoto = ["Lun", "Mar", "Mie", "Jue", "Vie", "Sab", "Dom"];
		this.diasLargo = ["Lunes", "Martes", "Miércoles", "Jueves", "Viernes", "Sábado", "Domingo"];
	}
	createCalendar(){
		this.startElements();
	...
	createHeaderM(){
		...
		var arrowL = $("<span class='btn prev_month "+this.btnH+"'>").html("<");
		var arrowR = $("<span class='btn next_month "+this.btnH+"'>").html(">");
		var title = $("<span class='current_month text-uppercase'>").html(moment(this.diaSeleccionado).format("MMMM - Y"));
		var _this = this;
	...
	createDaysMont(){
		...
		for(var i=0;i<this.cantDias;i++){
			...
			if(this.diaSeleccionado==fechNow){
				div = $("<div class='current-date-selected d-flex border flex-fill w-100 justify-content-center pt-3 pb-3 btn "+this.btnD+"' data-date='"+fechNow+"' style='background: "+this.colorResal2+"; color: "+this.textResalt+"; font-weight: bold;'>").html(i+1);
			}
